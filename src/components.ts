  ...
  DomComponents.addType("scale", {
    ...
    model: {
      ...
      init() {
        ...
        // Also update when attributes change (e.g., from loaded HTML)
        this.on("change:attributes", () => {
          const newAttr = this.getAttributes();
          const newPercent = parseInt(newAttr["data-percent"]);
          const newFcolor = newAttr["data-fcolor"];
          const newBgcolor = newAttr["data-bgcolor"];

          if (!isNaN(newPercent) && newPercent !== this.get("percent")) {
            this.set("percent", newPercent, { silent: true });
          }
          if (newFcolor && newFcolor !== this.get("fcolor")) {
            this.set("fcolor", newFcolor, { silent: true });
          }
          if (newBgcolor && newBgcolor !== this.get("bgcolor")) {
            this.set("bgcolor", newBgcolor, { silent: true });
          }
          this.updateScale();
        });
      },
