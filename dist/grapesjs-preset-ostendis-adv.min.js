...
var plugin = function (editor_1) {
    ...
    return __awaiter(void 0, __spreadArray([editor_1], args_1, true), void 0, function (editor, opts) {
        ...
        function cloneWithStyles(editor, component, insertAt) {
            var _a;
            var css = editor.CssComposer;
            var origToCloneMap = new Map();
            var originalComponents = getAllComponentsRecursive(component);
            var savedStyles = {};
            originalComponents.forEach(function (comp) {
                var id = comp.getId();
                var rule = css.getRule("#".concat(id));
                if (rule) {
                    savedStyles[id] = __assign({}, rule.getStyle());
                }
            });
            console.log(savedStyles);
            var clonedComponent = component.clone();
            var clonedComponents = getAllComponentsRecursive(clonedComponent);
            originalComponents.forEach(function (orig, idx) {
                var clone = clonedComponents[idx];
                if (clone) {
                    origToCloneMap.set(orig.getId(), clone.getId());
                }
            });
            origToCloneMap.forEach(function (newId, oldId) {
                var _a, _b;
                var style = savedStyles[oldId];
                if (style && Object.keys(style).length > 0) {
                    (_b = (_a = css).addRules) === null || _b === void 0 ? void 0 : _b.call(_a, [
                        {
                            selectors: ["#".concat(newId)],
            ...
            Object.entries(savedStyles).forEach(function (_a) {
                var id = _a[0], style = _a[1];
                var rule = css.getRule("#".concat(id)) ||
                    css.add([{ selectors: ["#".concat(id)], style: {} }])[0];
                rule.setStyle(__assign({}, style));
            });
            (_a = component.parent()) === null || _a === void 0 ? void 0 : _a.append(clonedComponent, { at: insertAt });
            return clonedComponent;
        }
        function getAllComponentsRecursive(component) {
            var result = [component];
            component.components().each(function (child) {
                result.push.apply(result, getAllComponentsRecursive(child));
            });
            return result;
        }
